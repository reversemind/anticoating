...
object Main extends App with LazyLogging {
  ...
  val queueService = actorSystem.actorOf(Props(new QueueApi()), name = "queueActor")
  val producerService = actorSystem.actorOf(Props(new MessageProducer(endPoint)), name = "messageProducerActor")

  val messageConsumerActor = actorSystem.actorOf(Props(new MessageConsumer(endPoint)), name = "messageConsumerActor")
  val messageProducerActor = actorSystem.actorOf(Props(classOf[CamelProducer], endPoint), name = "camelProducerActor")
  ...
  Thread.sleep(1000)
  messageProducerActor.tell(message, ActorRef.noSender)

  Thread.sleep(1000)
  producerService.tell(message, ActorRef.noSender)
